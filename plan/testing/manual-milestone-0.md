...
There is a dot but it is range and not cyan (blue)
...
I cant see where this could be
...
Almost I see `[y] approve [n] reject   [d] detail`
...
- [✅] `j`/`k` and the arrow keys scroll; `g`/`G` jump to top/bottom. I could not really test this as the modal was a bit small. I could test it further down
...
Other findings
When closing the modal the mani window remains greayed out but if I press esc it shows the colors again.
...
- [✅] `End` jumps to the bottom. Only way was to use shift g and g to go to end and top the home and end keys did not work on the keyboard
...
Scroll and pin. None of this works or it is not working as expected
...
When closing the modal the content is still greyed out I have to esc a asecond time for colors to return
